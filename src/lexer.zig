...
pub const Lexer = struct {
    ...
    fn checkKeyword(self: *Lexer, startIndex: usize, rest: []const u8, tokenType: TokenType) TokenType {
        return if (@ptrToInt(self.current) - @ptrToInt(self.start) == rest.len + startIndex and
            std.mem.eql(u8, rest, self.start[startIndex .. @ptrToInt(self.current) - @ptrToInt(self.start)])) tokenType else TokenType.IDENTIFIER;
    }
};
...
test "Scan Token" {
    ...
    try std.testing.expect(token.tokenType == TokenType.LEFT_PARENTHESIZE);
    token = lexer.scanToken();
    try std.testing.expect(token.tokenType == TokenType.RIGHT_PARENTHESIZE);
    token = lexer.scanToken();
    std.debug.print("token: {}\n", .{token.tokenType});
    try std.testing.expect(token.tokenType == TokenType.BANG_EQUAL);
}
...
test "Can handle comments" {
    ...
    var lexer = Lexer.init(source);
    var token = lexer.scanToken();
    try std.testing.expect(token.tokenType == TokenType.EOF);
}
...
test "Can handle strings" {
    ...
    var lexer = Lexer.init(source);
    var token = lexer.scanToken();
    try std.testing.expect(token.tokenType == TokenType.STRING);
    try std.testing.expect(token.length == 5);
}
